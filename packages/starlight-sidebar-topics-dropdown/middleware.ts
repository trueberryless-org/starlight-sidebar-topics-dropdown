...
export const onRequest = defineRouteMiddleware((context) => {
  ...
  Learn more about unlisted pages in the ["Unlisted pages"](https://starlight-sidebar-topics-dropdown.trueberryless.org/docs/guides/unlisted-pages/) guide.`
      );

    starlightRoute.sidebar = currentTopic.sidebar;
    // @ts-expect-error - See `libs/locals` for more information.
    context.locals[StarlightSidebarTopicsDropdownLocalsSymbol] = {
      config: currentTopic.config,
    } satisfies StarlightSidebarTopicsDropdownLocals;
