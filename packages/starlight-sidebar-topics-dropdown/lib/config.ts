...
const sidebarTopicBaseSchema = z.object({
  ...
  badge: z
    ...
    .transform((badge) =>
      typeof badge === "string"
        ? { variant: "default" as const, text: badge }
        : badge
    )
    .optional(),
  /**
...
const sidebarTopicGroupSchema = sidebarTopicBaseSchema.extend({
   ...
   */
  items: z.any().array() as z.Schema<
    NonNullable<StarlightUserConfig["sidebar"]>
  >,
});

export const StarlightSidebarTopicsDropdownConfigSchema = z
  .union([sidebarTopicGroupSchema, sidebarTopicLinkSchema])
  .array();

export type StarlightSidebarTopicsDropdownUserConfig = z.input<
  typeof StarlightSidebarTopicsDropdownConfigSchema
>;
export type StarlightSidebarTopicsDropdownConfig = z.output<
  typeof StarlightSidebarTopicsDropdownConfigSchema
>;

export type StarlightSidebarTopicsDropdownSharedConfig = (
